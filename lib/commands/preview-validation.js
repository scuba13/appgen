...
function previewTasks(runPreview) {
  return [
    ...
    { id: 'prepare-compose', label: 'Preparar Docker Compose e relatorio' },
    {
      id: 'run-smoke-test',
      label: runPreview
        ? 'Subir Docker Compose e executar smoke test'
        : 'Validar compose sem subir containers',
    },
  ];
}
...
export default async function previewValidation(args) {
  ...
  if (!existsSync(statePath)) {
    ...
    console.log('  Run ' + chalk.bold('npx appgen install') + ' before preview-validation.\n');
    return;
  }
  ...
  if (!existsSync(appRootPath)) {
    state = updateTask({
      ...
      status: 'blocked',
      message: `${appRoot} nao existe. Rode appgen scaffold antes.`,
    });
    console.log(chalk.yellow(`\n  App root not found: ${appRoot}`));
    console.log('  Run ' + chalk.bold('appgen scaffold') + ' before preview-validation.\n');
    return;
  }
  state = updateTask({
    statePath,
  ...
  if (state.preview_validation.status === 'ready_for_user_test') {
    ...
    console.log(`  API health:      ${chalk.cyan('http://localhost:3001/health')}`);
    console.log('  Next step:       appgen acceptance');
  } else if (result.status === 'blocked_environment') {
    console.log(chalk.yellow('  Docker/Compose is required before user acceptance.'));
    console.log(`  Suggested:       ${result.installPlan.command}`);
    console.log(`  Note:            ${result.installPlan.note}`);
  } else {
    console.log(chalk.yellow('  Fix the preview failures before asking the user to test.'));
  }
