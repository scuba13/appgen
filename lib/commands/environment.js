...
import { environmentBlocker } from '../runtime/business-experience.js';
...
function updateTask({ statePath, state, taskId, status, message = null, extra = {} }) {
  ...
  if (task) {
    ...
    if (status !== 'pending' && !task.started_at) task.started_at = nowIso();
...
function syncEnvironmentFlow(state) {
  ...
  completed.delete('environment');
  const nextPending = pending.includes('environment')
    ? pending
    : ['environment', ...pending.filter(step => step !== 'environment')];
  const checkpoints = { ...(state.checkpoints ?? {}) };
  delete checkpoints['appgen-environment'];
...
function markEnvironmentPending(projectRoot) {
  const planPath = join(projectRoot, '.appgen', 'plan.md');
  if (!existsSync(planPath)) return;
  const content = readFileSync(planPath, 'utf8');
  const updated = content.replace('- [x] appgen-environment', '- [ ] appgen-environment');
  if (updated !== content) {
...
function actionPlan({ env, dockerDaemonReady, installPlan }) {
  if (!env.docker.installed) {
    return {
      kind: 'install_docker',
      title: 'Docker nao foi detectado.',
      action: `Instale Docker Desktop com autorizacao explicita ou rode node .appgen/bin/appgen.js environment --install-docker. Sugestao: ${installPlan.command}`,
      command: 'node .appgen/bin/appgen.js environment --install-docker',
      note: installPlan.note,
    };
  }

  if (!env.compose.installed) {
    return {
      kind: 'install_compose',
      title: 'Docker Compose nao foi detectado.',
      ...
      note: 'Docker Compose normalmente vem junto com Docker Desktop.',
    };
  }

  if (!dockerDaemonReady) {
    return {
      kind: 'open_docker_desktop',
      title: 'Docker esta instalado, mas o engine nao esta ativo.',
      action: 'Abra o Docker Desktop, aguarde o engine iniciar e rode node .appgen/bin/appgen.js environment novamente.',
      command: 'node .appgen/bin/appgen.js environment',
      note: 'Nao e necessario reinstalar Docker quando o problema for apenas o daemon desligado.',
    };
  }

  return {
    kind: 'ready',
    title: 'Docker e Docker Compose estao prontos.',
    action: 'Pode seguir para appgen-specs.',
    command: 'node .appgen/bin/appgen.js next',
    note: 'Ambiente local minimo validado.',
  };
}

function reportMarkdown({ state, env, dockerDaemonReady, tools, containers, status, plan }) {
...
${env.docker.installed
  ...
  : 'Docker nao foi detectado na maquina.'}
...
async function installDockerIfApproved({ chalk, installPlan, assumeYes }) {
  ...
  try {
    ...
    return { status: 'installer_completed', message: 'Docker installer completed. Open Docker Desktop and rerun node .appgen/bin/appgen.js environment.' };
  } catch (error) {
...
export default async function environment(args) {
  ...
  if (!existsSync(statePath)) {
    ...
    console.log('  Run ' + chalk.bold('npx appgen-ai install') + ' before environment.\n');
    return;
  }
  ...
  const plan = actionPlan({ env, dockerDaemonReady, installPlan });
  ...
  state = updateTask({
    ...
    extra: {
      ...
      install_plan: !env.docker.installed && dockerStatus !== 'done' ? installPlan : null,
      action_plan: plan,
    },
  });
  ...
  state = syncEnvironmentFlow(state);
  if (state.environment.status !== 'ready') {
    markEnvironmentPending(projectRoot);
  }
  mkdirSync(dirname(reportPath), { recursive: true });
  writeFileSync(
    reportPath,
  ...
  if (state.environment.status !== 'ready') {
    ...
    console.log('');
    console.log(chalk.yellow(`  ${blocker?.title || plan.title}`));
    console.log(`  Action:          ${blocker?.action || plan.action}`);
    console.log(`  Command:         ${chalk.bold(blocker?.command || plan.command)}`);
