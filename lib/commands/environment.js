...
function environmentTasks() {
  return [
    { id: 'detect-tools', label: 'Detectar ferramentas locais minimas' },
    { id: 'validate-docker', label: 'Validar Docker e Docker Compose' },
    { id: 'plan-containers', label: 'Planejar ambiente containerizado' },
  ];
}
...
export default async function environment(args) {
  ...
  if (state.environment.status !== 'ready') {
    console.log('');
    console.log(chalk.yellow('  O ambiente containerizado ainda nao esta pronto.'));
