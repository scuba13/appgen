...
async function installDockerIfApproved({ chalk, env, installPlan, assumeYes }) {
  ...
  try {
    ...
    return {
      ...
      message: 'Installer command completed. Open Docker Desktop and wait for the engine to start, then rerun appgen acceptance --prepare.',
    };
  } catch (error) {
    return {
      attempted: true,
...
export default async function acceptance(args) {
  ...
  if (!existsSync(statePath)) {
    ...
    console.log('  Run ' + chalk.bold('npx appgen install') + ' before acceptance.\n');
    return;
  }
  ...
  if (feedback) {
    ...
    console.log(chalk.yellow(`\n  Feedback recorded as ${normalizedType}. Recommended route: ${nextStep}.\n`));
    return;
  }
...
`,
  ...
  if (ready) {
    ...
    console.log(`  Approve with:   ${chalk.bold('appgen acceptance --ok')}`);
  } else {
    console.log('');
    ...
    console.log('  Installation must be explicitly approved by the user before running any installer command.');
  }
  console.log('');
}
