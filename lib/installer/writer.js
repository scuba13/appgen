...
const CLI_PATH = join(REPO_ROOT, 'bin', 'appgen.js');
...
export class Writer {
  ...
  async installEntryFile(engine, { force = false, conflictStrategy = null } = {}) {
    ...
    // Arquivo já existe — perguntar ao usuário (apenas merge ou skip), exceto em modo CI.
    const strategy = conflictStrategy || await (async () => {
      const { askMergeStrategy } = await import('./prompts.js');
      return askMergeStrategy(engine.entryFile);
    })();
  ...
  async installCommandFiles(engine, { force = false } = {}) {
    for (const command of engine.commandTemplates ?? []) {
      const templatePath = join(TEMPLATES_DIR, 'commands', command.template);
      const destPath = join(this.projectRoot, command.target);
  ...
  installCliRunner({ force = false } = {}) {
    const runnerPath = join(this.projectRoot, '.appgen', 'bin', 'appgen.js');
    if (existsSync(runnerPath) && !force) return false;
...
`;
    ...
    this._mkdir(dirname(runnerPath));
    writeFileSync(runnerPath, content, 'utf8');
    chmodSync(runnerPath, 0o755);
    this._register(runnerPath);
    return true;
  }
