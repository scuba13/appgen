...
export function copyPresetScaffold({
  ...
  appendActivityLog(projectRoot, { work_folder: workDir }, startEvent);
  ...
  for (const src of files) {
    ...
    if (existsSync(dest) && !overwrite) {
      ...
      appendActivityLog(projectRoot, { work_folder: workDir }, skippedEvent);
      onProgress?.({
        status: 'skipped',
        target: join(appRoot, relTarget),
        reason: 'exists',
    ...
    appendActivityLog(projectRoot, { work_folder: workDir }, createdEvent);
    onProgress?.({
      status: 'created',
      target: join(appRoot, relTarget),
      files: [join(appRoot, relTarget)],
  ...
  appendActivityLog(projectRoot, { work_folder: workDir }, doneEvent);
