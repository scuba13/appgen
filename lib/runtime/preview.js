...
async function waitForFirstHealthyUrl(urls, timeoutMs = 60000) {
  const startedAt = Date.now();
  const errors = [];
  ...
  return { status: 'failed', url: urls.join(' or '), error: errors.at(-1) || 'timeout' };
}
...
- API health v1: http://localhost:3001/api/v1/health
...
export async function preparePreview({ projectRoot, state, force = false, runPreview = false }) {
  ...
  if (upCheck.status === 'passed') {
    const apiCheck = await waitForFirstHealthyUrl([
    ...
    if (webCheck.status === 'passed') {
      const e2eCheck = runCommand('docker', ['compose', 'run', '--rm', 'e2e'], { cwd: appRootPath, timeoutMs: 180000 });
      checks.push({
        name: 'Playwright E2E Docker',
        status: e2eCheck.status,
        detail: e2eCheck.status === 'passed' ? 'browser smoke validado no container e2e' : e2eCheck.stderr || e2eCheck.stdout || 'falha no e2e containerizado',
        command: e2eCheck.command,
      });
    }
  }
