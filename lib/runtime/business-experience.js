...
function featureSliceRows(markdown, limit = 6) {
  const technicalPattern = /\b(fundacao|fundação|scaffold|polimento|validacao|validação|quality|qa|docs|handoff|setup|infra)\b/i;

  return markdown
    .split('\n')
    .map(line => line.trim())
    .filter(line => /^\|\s*S\d{3,}\s*\|/.test(line))
    .map(line => {
    ...
    .filter(slice => slice.id && (slice.name || slice.objective))
    .filter(slice => !technicalPattern.test(`${slice.name} ${slice.objective}`))
    .slice(0, limit);
}
...
export function writeAcceptanceGuide(projectRoot, state) {
  ...
  const sliceFlows = featureSliceRows(slices, 6);
  const users = itemsFrom(product, ['usuario', 'usuário', 'persona', 'ator', 'perfil'], 5);
  const flows = [
    ...sliceFlows.map(slice => `${slice.name}: ${slice.objective || 'validar o comportamento principal da funcionalidade.'}`),
  ...
  const flowSteps = flows.length
    ? flows.map((flow, index) => `${index + 1}. Execute o fluxo: ${flow}\n   - Resultado esperado: o usuario consegue concluir a acao sem atalhos tecnicos, dados falsos obrigatorios ou erro inesperado.`).join('\n')
  ...
  const personaSteps = users.length
    ? users.map(user => `- ${user}`).join('\n')
    : '- Use o perfil principal descrito no produto e valide o fluxo como uma pessoa de negocio, nao como desenvolvedor.';
...
## Sinais De Problema Para Reportar

- A tela nao permite concluir o fluxo esperado.
- Uma regra de negocio esta errada, ausente ou dificil de conferir.
- A app mostra erro tecnico para uma acao comum.
- O layout impede leitura, clique ou uso em desktop/mobile.
- O ambiente nao abriu, caiu ou ficou indisponivel durante o teste.
