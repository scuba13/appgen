...
import { environmentBlocker } from './business-experience.js';
...
export const APPGEN_STEPS = [
  ...
  { id: 'environment', agent: 'appgen-environment', files: ['_appgen_work/environment-report.md'] },
  {
    id: 'specs',
    agent: 'appgen-specs',
    files: [
...
export function buildInitialImplementationLoop(state = {}) {
  ...
  return {
    ...
    slice_iteration: existing.slice_iteration ?? existing.iteration ?? 0,
    ...
    pause_between_slices: existing.pause_between_slices ?? state.runtime?.pause_between_slices ?? true,
    awaiting_user_decision: existing.awaiting_user_decision ?? false,
...
export function completeStep(projectRoot, stepOrAgent, files = []) {
  ...
  try {
    ...
    if (step.id === 'environment' && state.environment?.status !== 'ready') {
      const blocker = environmentBlocker(state);
      throw new Error(
        blocker
          ? `Cannot complete "environment": ${blocker.action}`
          : 'Cannot complete "environment": environment status must be ready.'
      );
    }
